(function($) {
    ...
    couponInputButton =  'table.cart td.actions .coupon .button';
        ...
        $(couponClick).on('click', function(e){

            if($(this).hasClass("enabled")) {

                var code = jQuery(this).children(".code").text();
                $(this).removeClass('enabled');
                $(this).addClass('disabled');

                $(couponInputText).val(code);
                $(couponInputButton).click();
            }

        });

    $(".cart_totals").live( 'DOMSubtreeModified',function(){ // отслеживаем изменение содержимого блока
        //jQuery(".block_3").text( bilo + ' -> ' + stalo ); // вставляем в блок значения переменных
        //alert (1);
